!> Uniform flow along a uniform frozen field, on a spherical mesh.
!>
...
!> uniform-flow states are for tests/hd,mhd,srhd/spherical_uniform_flow --
...
!> exercise is that b-hat's spherical components, which vary with theta and
!> phi even though the physical direction is constant, are handled correctly
!> by the ordinary curvilinear flux machinery.
module mod_usr
  use mod_amrvac
  use mod_physics
...
contains
  ...
  subroutine specialbound_usr(qt, ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,&
    ...
    !$acc loop collapse(3) vector private(bhat)
