!> Field-aligned pressure pulse on a spherical mesh, threaded by a uniform
...
!> tests/ffhd/spherical_uniform_flow (Cartesian, so it is a non-trivial
!> function of theta and phi once written in spherical components). Unlike
!> a full HD/MHD blast, ffhd only has a single field-aligned momentum
!> component, so the perturbation can only launch waves along the (curved,
!> spatially varying) field direction rather than in every direction; this
!> is a shock/AMR regression test rather than a well-balancing one
!> (tests/ffhd/spherical_uniform_flow covers that, including the frozen
    ...
    do ix3 = ixmin3, ixmax3
       do ix2 = ixmin2, ixmax2
          do ix1 = ixmin1, ixmax1
             ...
             call to_spherical_unit(x(ix1,ix2,ix3,1:ndim), b0, bhat)
