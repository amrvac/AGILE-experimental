module mod_usr
  ...
  ! Zhou et al. (2025, ApJ 978:72), Sect. 2.3: modified oblique ring conduction test
  ! (Runs RB1/RB2). Solar-coronal version of the ring test: physical domain
  ! [-10,10] Mm, ring at 5 Mm < r < 7 Mm, T = 1 MK inside the hot arc, 0.01 MK
  ! outside, and temperature-dependent (Spitzer) parallel conductivity.
  !
  ! With unit_length = 1e9 cm (=10 Mm) and unit_temperature = 1e6 K, the domain
  ! [-1,1] and ring radii [0.5,0.7] in code units map exactly to the physical values we want
  double precision, parameter :: ring_rmin    = 0.5d0    ! 5 Mm
  double precision, parameter :: ring_rmax    = 0.7d0    ! 7 Mm
  double precision, parameter :: ring_slab_hw = 0.2d0    ! half-width in ring-frame z
  double precision, parameter :: T_hot        = 1.0d0    ! 1 MK   (coronal)
  double precision, parameter :: T_cold       = 0.01d0   ! 0.01 MK (prominence)
  double precision, parameter :: ring_Busr    = 1.0d-5   ! B = ring_Busr/r (line-current field, eq. 63)

  ! Parallel-conduction equilibrium of the ring: the 30-deg hot arc spans 1/12 of
  ! the azimuth, so the fully-mixed ring temperature is
  !   (1*T_hot + 11*T_cold)/12 = (1 + 11*0.01)/12 = 0.0925 MK.
  double precision, parameter :: T_ring_eq    = (T_hot + 11.0d0*T_cold)/12.0d0
...
contains
  ...
  subroutine usr_init()
    ...
    ! Leaving tc_kappa_par unset in the .par selects Spitzer sig_par = tc_kappa0_par*Te^2.5,
    ! with tc_kappa0_par derived from these units in phys_units().
    unit_length        = 1.0d9   ! cm  (10 Mm)
    unit_temperature   = 1.0d6   ! K   (1 MK)
    unit_numberdensity = 1.0d9   ! cm^-3 (coronal)
    ...
    where (r(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3) > ring_rmin &
     .and. r(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3) < ring_rmax)
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+2) = &
         dabs(T(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3) - T_ring_eq)
    elsewhere
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,nw+2) = &
