!> Uniform Cartesian MHD flow on a cylindrical mesh.
!>
...
!> steady solution of the MHD equations. Written in cylindrical components,
!> both the velocity and field are non-trivial functions of phi (only the
!> axial components stay literally constant), so keeping this state exactly
!> uniform exercises every curvilinear flux and every geometric source term in
!> addsource_geometry, including the induction equation and GLM psi coupling
!> terms. Any error in them shows up immediately as drift in the volume
!> averages the log file reports.
module mod_usr
  use mod_amrvac
  use mod_physics
