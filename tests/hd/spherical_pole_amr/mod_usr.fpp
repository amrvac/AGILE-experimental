!> Uniform Cartesian flow across both poles, on a two-level AMR mesh.
!>
!> A constant density, constant pressure gas moving with a constant Cartesian
!> velocity is an exact steady solution of the Euler equations. Written in
!> spherical components the velocity is a non-trivial function of theta and phi,
!> so keeping this state exactly uniform exercises every curvilinear flux and
...
!>
!> Half the domain in phi is refined as well (see usr_refine_grid), so that
!> blocks meet across the axis at different levels and the restricting and
!> prolonging pole paths of the ghost-cell exchange are exercised, not just the
!> same-level one.
module mod_usr
  use mod_amrvac
  use mod_physics
...
contains
  ...
  subroutine check_pole_ghosts(igrid,level,ixImin1,ixImin2,ixImin3,ixImax1,&
    ...
    do iside = 1, 2
       ...
       ! level jump the values are restricted or prolonged on the way, and
       ! carry the scheme's own second-order error; the loose tolerance there
       ! still catches what actually goes wrong in that path - a mistaken
       ! destination offset or a missing sign flip, both of which are O(1).
  ...
  !> dependency (refine_usr = .true. in &meshlist turns it on).
  !>
  !> The point is the level jump this leaves along phi = 0 and phi = pi: a
  !> block touching the axis inside the wedge has its pi-periodic partner
  !> outside it, one level coarser, so the pole copy has to go through the
  !> restriction and prolongation paths rather than the same-level one.
