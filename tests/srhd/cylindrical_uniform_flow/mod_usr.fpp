!> Uniform Cartesian relativistic flow on a cylindrical mesh.
!>
...
!> cylindrical components the radial and azimuthal velocity are non-trivial
!> functions of phi (only the axial velocity stays literally constant), so
!> keeping this state exactly uniform exercises every curvilinear flux and
!> every geometric source term. Any error in them shows up immediately as
!> drift in the volume averages the log file reports.
!>
!> The Lorentz factor only depends on |v|, which a cylindrical rotation of a
