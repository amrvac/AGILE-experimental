...
'mcbeta': A second-order symmetric TVD limiter with a parameter \(\beta\) whose default value is 1.4. Some other usually used values could be 1.5 (see the H-AMR code) or 1.9 (see the KORAL code). In principle, this \(\beta\) could be changed from 1 to 2. If \(\beta\) = 1, it will degenerate into the 'minmod' limiter and if \(\beta\) = 2, it will become the 'woodward' limiter.

'albada': Also a classic second-order symmetric TVD limiter. Similar with the 'vanleer' limiter but smoother. Note that \(\Delta^2\) is set to be 1.d-12 in our code to avoid clipping smooth extrema. However, a better idea to set \(\Delta^2\) is to make it proportional to \(\Delta x^3\), as we can see in the settings of the 'venkatakrishnan' limiter or 'wenoyc3' limiter.
