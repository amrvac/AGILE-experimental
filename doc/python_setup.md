...
All Python tools are located in the directory `$AGILE_DIR/tools/python`. This folder contains a setup
script to install the package, a small readme with some instructions and finally the subdirectory `agile_pytools` itself,
...
```bash
         export PYTHONPATH=$AGILE_DIR/tools/python:$PYTHONPATH
```
   * [Windows] Windows users can modify the environment variables using the control panel:
     ...
     full path to `.../agile/tools/python`. Multiple paths for a single environment variable are separated by a semicolon ';'.
