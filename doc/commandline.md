...
which currently prints:
    ...
    Optional arguments:
    -convert             Convert datfiles
    -if file0001.dat     Use this datfile to restart from
                         (you can modify e.g. output names)
    -resume              Automatically resume previous run
                         (useful for long runs on HPC systems)
    -datfilenext N       Manual index for next datfile
...
This will automatically look for the last available datfile (`.dat`) and
...
datfile from which the run resumed, so that the whole data series keep
the same pace as if it was never stopped and resumed.

## Restart a previous simulation from a specific datfile

    mpirun -np 4 ./amrvac -i amrvac.par -if output/sim0010.dat

To start the new output from a different index:

    mpirun -np 4 ./amrvac -i amrvac.par -if output/sim0010.dat -datfilenext 100
