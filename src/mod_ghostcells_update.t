...
contains
  ...
  subroutine getbc(time,qdt,psb,nwstart,nwbc,req_diag)
    ...
    use mod_boundary_conditions, only: getintbc
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      {do i^DB=-1,1\}
         ...
         case (neighbor_sibling)
            call bc_recv_srl(igrid,psb,i^D)
         case (neighbor_fine)
            call bc_recv_restrict(igrid,psb,i^D)
         end select
      {end do\}
    end do
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      {do i^DB=-1,1\}
         select case (neighbor_type(i^D,igrid))
         case (neighbor_sibling)
            call bc_send_srl(igrid,psb,i^D,nwbc,nwhead,nwtail)
         case (neighbor_coarse)
            call bc_send_restrict(igrid,psb,i^D,nwbc,nwhead,nwtail)
         end select
      {end do\}
    end do
    ...
    !$acc parallel loop default(present) copyin(ixS_srl_^L,ixR_srl_^L) private(igrid,i^D)
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      !$acc loop seq
      {do i^DB=-1,1\}
         select case (neighbor_type(i^D,igrid))
         case(neighbor_sibling)
           call bc_fill_srl(igrid,psb,i^D,nwhead,nwtail)
         case(neighbor_coarse)
           call bc_fill_restrict(igrid,psb,i^D,nwhead,nwtail)
         end select
      {end do\}
    end do
    ...
    if(stagger_grid) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
       {do i^DB=-1,1\}
          select case (neighbor_type(i^D,igrid))
          case (neighbor_sibling)
             call bc_fill_srl_stg(igrid,psb,i^D)
          case (neighbor_fine)
             call bc_fill_restrict_stg(igrid,psb,i^D)
          end select
       {end do\}
      end do
    end if
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      {do i^DB=-1,1\}
         if (neighbor_type(i^D,igrid)==neighbor_coarse) call bc_recv_prolong(igrid,psb,i^D)
      {end do\}
    end do
    ! MPI send ghost-cell values to finer neighbors in different processors
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      {do i^DB=-1,1\}
         if (neighbor_type(i^D,igrid)==neighbor_fine) call bc_send_prolong(igrid,psb,i^D,nwbc,nwhead,nwtail)
      {end do\}
    end do
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      {do i^DB=-1,1\}
         if (neighbor_type(i^D,igrid)==neighbor_fine) call bc_fill_prolong(igrid,psb,i^D,nwhead,nwtail)
      {end do\}
    end do
    !$OMP END PARALLEL SECTIONS
    ...
    !$OMP PARALLEL DO SCHEDULE(dynamic) PRIVATE(igrid)
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      call gc_prolong(igrid,psb,time,nwhead,nwtail)
    end do
    !$OMP END PARALLEL DO
    ...
    if(bcphys) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
        ...
        call fill_boundary_after_gc(igrid,psb,time,qdt)
      end do
      !$OMP END PARALLEL DO
    end if
  ...
  subroutine fill_boundary_after_gc(igrid,psb,time,qdt)
    use mod_global_parameters
  ...
  subroutine bc_recv_srl(igrid,psb,i^D)
    use mod_global_parameters
  ...
  subroutine bc_recv_restrict(igrid,psb,i^D)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D
    type(state), target :: psb(max_blocks)
  ...
  subroutine bc_send_srl(igrid,psb,i^D,nwbc,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwbc,nwhead,nwtail
    type(state), target :: psb(max_blocks)

    integer :: n_i^D,ipole,idir,ineighbor,ipe_neighbor,ixS^L, isizes
    ...
    if(ipe_neighbor/=mype) then
      ...
      if(ipole==0) then
        ...
        call pole_buffer(pwbuf(ipwbuf)%w,ixS^L,ixS^L,psb(igrid)%w,ixG^LL,ixS^L,ipole,i^D,nwhead,nwtail)
        isend_c=isend_c+1
  ...
  subroutine bc_send_restrict(igrid,psb,i^D,nwbc,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwbc,nwhead,nwtail
    type(state), target :: psb(max_blocks)

    integer :: ic^D,n_inc^D,ipole,idir,ineighbor,ipe_neighbor,ixS^L,isizes
    integer :: ibuf_start, ibuf_next
    integer, dimension(1) :: shapes
    ...
    if(ipe_neighbor/=mype) then
      ...
      if(ipole==0) then
        ...
        allocate(pwbuf(ipwbuf)%w(ixS^S,nwhead:nwtail))
        call pole_buffer(pwbuf(ipwbuf)%w,ixS^L,ixS^L,psc(igrid)%w,ixCoG^L,ixS^L,ipole,i^D,nwhead,nwtail)
        isend_c=isend_c+1
        isend_buf(ipwbuf)=isend_c
        itag=(3**^ND+4**^ND)*(ineighbor-1)+3**^ND+{n_inc^D*4**(^D-1)+}
        isizes={(ixSmax^D-ixSmin^D+1)*}*nwbc
  ...
  subroutine bc_fill_srl(igrid,psb,i^D,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwhead,nwtail
    type(state), target :: psb(max_blocks)

    integer :: ineighbor,ipe_neighbor,ipole,ixS^L,ixR^L,n_i^D,idir,iw,ix^D
    ...
    if(ipe_neighbor==mype) then
      ...
      if(ipole==0) then
        ...
        do iw=nwhead,nwtail
         {do ix^DB=1,ixSmax^DB-ixSmin^DB+1\}
        ...
        if(stagger_grid) then
          do idir=1,ndim
           ...
           {do ix^DB=1,ixSmax^DB-ixSmin^DB+1\}
             psb(ineighbor)%ws({ixRmin^D+ix^D-1},idir)=psb(igrid)%ws({ixSmin^D+ix^D-1},idir)
           {end do\}
          end do
        end if
      else
        ...
        ixR^L=ixR_srl_^L(n_i^D);
        call pole_copy(psb(ineighbor)%w,ixG^LL,ixR^L,psb(igrid)%w,ixG^LL,ixS^L,ipole,i^D,nwhead,nwtail)
        if(stagger_grid) then
          do idir=1,ndim
            ixS^L=ixS_srl_stg_^L(idir,i^D);
            ixR^L=ixR_srl_stg_^L(idir,n_i^D);
            call pole_copy_stg(psb(ineighbor)%ws,ixGs^LL,ixR^L,psb(igrid)%ws,ixGs^LL,ixS^L,idir,ipole,i^D)
          end do
        end if
      end if
    end if
  ...
  subroutine bc_fill_restrict(igrid,psb,i^D,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwhead,nwtail
    type(state), target :: psb(max_blocks)
    ...
    if(ipe_neighbor==mype) then
      ...
      if(ipole==0) then
        ...
        ixR^L=ixR_r_^L(n_inc^D);
        call pole_copy(psb(ineighbor)%w,ixG^LL,ixR^L,psc(igrid)%w,ixCoG^L,ixS^L,ipole,i^D,nwhead,nwtail)
        if(stagger_grid) then
          do idir=1,ndim
            ixS^L=ixS_r_stg_^L(idir,i^D);
            ixR^L=ixR_r_stg_^L(idir,n_inc^D);
            !! Fill ghost cells
            call pole_copy_stg(psb(ineighbor)%ws,ixGs^LL,ixR^L,psc(igrid)%ws,ixCoGs^L,ixS^L,idir,ipole,i^D)
          end do
        end if
      end if
    end if
  ...
  subroutine bc_fill_srl_stg(igrid,psb,i^D)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D
    type(state), target :: psb(max_blocks)

    integer :: ixS^L,ixR^L,n_i^D,idir,ineighbor,ipe_neighbor,ipole,ibuf_next
      ...
      if (ipole==0) then
        ...
        do idir=1,ndim
         ...
         pole_buf%ws(ixS^S,idir)=reshape(source=recvbuffer_srl(ibuf_recv_srl:ibuf_next-1),&
           shape=shape(psb(igrid)%ws(ixS^S,idir)))
         ibuf_recv_srl=ibuf_next
         call pole_copy_stg(psb(igrid)%ws,ixGs^LL,ixR^L,pole_buf%ws,ixGs^LL,ixS^L,idir,ipole,i^D)
        end do
      end if
    end if
  ...
  subroutine bc_fill_restrict_stg(igrid,psb,i^D)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D
    type(state), target :: psb(max_blocks)

    integer :: ipole,ic^D,inc^D,ineighbor,ipe_neighbor,ixS^L,ixR^L,n_i^D,idir,ibuf_next
          ...
          {case (^D)
           ...
           do idir=1,ndim
             ...
             pole_buf%ws(ixR^S,idir)=reshape(source=recvbuffer_r(ibuf_recv_r:ibuf_next-1),&
               shape=shape(psb(igrid)%ws(ixR^S,idir)))
             call pole_copy_stg(psb(igrid)%ws,ixGs^LL,ixR^L,pole_buf%ws,ixGs^LL,ixR^L,idir,ipole,i^D)
             ibuf_recv_r=ibuf_next
           end do
         end if
      {end do\}
  ...
  subroutine bc_recv_prolong(igrid,psb,i^D)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D
    type(state), target :: psb(max_blocks)
  ...
  subroutine bc_send_prolong(igrid,psb,i^D,nwbc,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwbc,nwhead,nwtail
    type(state), target :: psb(max_blocks)

    integer :: ic^D,inc^D,n_i^D,n_inc^D,ineighbor,ipe_neighbor,ixS^L,ipole,idir,isizes
    integer :: ibuf_start, ibuf_next
    integer, dimension(1) :: shapes
  ...
  !> Send to finer neighbor
  subroutine bc_fill_prolong(igrid,psb,i^D,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,i^D,nwhead,nwtail
    type(state), target :: psb(max_blocks)
    ...
    if(ipole==0) then
      ...
      {do ic^DB=1+int((1-i^DB)/2),2-int((1+i^DB)/2)
         ...
         if(ipe_neighbor==mype) then
           ...
           ixR^L=ixR_p_^L(n_inc^D);
           call pole_copy(psc(ineighbor)%w,ixCoG^L,ixR^L,psb(igrid)%w,ixG^LL,ixS^L,ipole,i^D,nwhead,nwtail)
           if(stagger_grid) then
             do idir=1,ndim
               ixS^L=ixS_p_stg_^L(idir,inc^D);
               ixR^L=ixR_p_stg_^L(idir,n_inc^D);
               call pole_copy_stg(psc(ineighbor)%ws,ixCoGs^L,ixR^L,psb(igrid)%ws,ixGs^LL,ixS^L,idir,ipole,i^D)
             end do
           end if
         end if
      {end do\}
  ...
  subroutine gc_prolong(igrid,psb,time,nwhead,nwtail)
    use mod_global_parameters
    integer, intent(in) :: igrid,nwhead,nwtail
    type(state), target :: psb(max_blocks)
    ...
    {do i^DB=-1,1\}
       if (neighbor_type(i^D,igrid)==neighbor_coarse) then
         call bc_prolong(igrid,psb,time,i^D,nwhead,nwtail)
    ...
    if(stagger_grid) then
      ...
      do idims=1,ndim
       ...
       {case(^D)
          do i^D=-1,1,2
            if (NeedProlong(i^DD)) call bc_prolong_stg(igrid,psb,i^DD,NeedProlong)
          end do
        \}
        end select
      end do
      ...
      do i2=-1,1,2
        do i3=-1,1,2
          if (NeedProlong(i^D)) call bc_prolong_stg(igrid,psb,i^D,NeedProlong)
        end do
      end do
      i2=0;
      do i3=-1,1,2
  ...
  subroutine bc_fill_prolong_stg(igrid,i^D)
    ...
    integer :: ipe_neighbor,ineighbor,ipole,ixR^L,ic^D,inc^D,n_inc^D,idir,ibuf_next
      ...
      if (ipole==0) then   !! There is no pole 
        ...
        do idir=1,ndim
          ...
          pole_buf%ws(ixR^S,idir)=reshape(source=recvbuffer_p(ibuf_recv_p:ibuf_next-1),&
            shape=shape(psc(igrid)%ws(ixR^S,idir)))
          call pole_copy_stg(psc(igrid)%ws,ixCoGs^L,ixR^L,pole_buf%ws,ixGs^LL,ixR^L,idir,ipole,i^D)
          ibuf_recv_p=ibuf_next
        end do
      end if
    end if
  ...
  subroutine bc_prolong(igrid,psb,time,i^D,nwhead,nwtail)
    use mod_global_parameters
    ...
    if(ghost_copy) then
      call interpolation_copy(igrid,psb,ixFi^L,dxFi^D,xFimin^D,dxCo^D,invdxCo^D,xComin^D,nwhead,nwtail)
    else
      call interpolation_linear(igrid,psb,ixFi^L,dxFi^D,xFimin^D,dxCo^D,invdxCo^D,xComin^D,nwhead,nwtail)
    end if
  ...
  subroutine bc_prolong_stg(igrid,psb,i^D,NeedProlong)
    use mod_global_parameters
    use mod_amr_fct
    use mod_physics, only: phys_to_primitive, phys_to_conserved
  ...
  subroutine interpolation_linear(igrid,psb,ixFi^L,dxFi^D,xFimin^D, &
                                  dxCo^D,invdxCo^D,xComin^D,nwhead,nwtail)
    use mod_global_parameters
    use mod_physics, only: phys_to_conserved
    integer, intent(in) :: igrid, ixFi^L,nwhead,nwtail
    type(state), target :: psb(max_blocks)
  ...
  subroutine interpolation_copy(igrid,psb,ixFi^L,dxFi^D,xFimin^D, &
                                dxCo^D,invdxCo^D,xComin^D,nwhead,nwtail)
    use mod_global_parameters
    use mod_physics, only: phys_to_conserved
    integer, intent(in) :: igrid, ixFi^L,nwhead,nwtail
  ...
  subroutine pole_copy(wrecv,ixIR^L,ixR^L,wsend,ixIS^L,ixS^L,ipole,i^D,nwhead,nwtail)
    use mod_global_parameters
    use mod_comm_lib, only: mpistop
  
    integer, intent(in) :: ixIR^L,ixR^L,ixIS^L,ixS^L,ipole,i^D,nwhead,nwtail
  ...
  subroutine pole_copy_stg(wrecv,ixIR^L,ixR^L,wsend,ixIS^L,ixS^L,idirs,ipole,i^D)
    use mod_global_parameters
    use mod_comm_lib, only: mpistop
  
    integer, intent(in) :: ixIR^L,ixR^L,ixIS^L,ixS^L,idirs,ipole,i^D
  ...
  subroutine pole_buffer(wrecv,ixIR^L,ixR^L,wsend,ixIS^L,ixS^L,ipole,i^D,nwhead,nwtail)
    use mod_global_parameters
    use mod_comm_lib, only: mpistop
