...
! Debugging is only intended to work on CPU.
  ...
  double precision :: t_JCcorona(1:45), t_DM_2(1:76), t_Colgan(1:55), t_custom(1:100)
  double precision :: l_JCcorona(1:45), l_DM_2(1:76), l_Colgan(1:55), l_custom(1:100)
  integer          :: n_JCcorona, n_DM_2 , n_Colgan, n_custom
  ...
  data n_custom / 50 /
  data t_custom / 1.0        , 1.08080808, 1.16161616, 1.24242424, 1.32323232,&
  ...
  data l_custom / -50.0        , -50.0        , -50.0        , -50.0        ,&
       -50.0        , -50.0        , -50.0        , -50.0        ,&
       -50.0        , -50.0        , -50.0        , -50.0        ,&
       -50.0        , -50.0        , -50.0        , -50.0        ,&
       -50.0        , -50.0        , -50.0        , -50.0        ,&
  ...
  @:get_Rfactor()
    ...
    subroutine radiative_cooling_init(fl)
      ...
      case('custom')
        if(mype==0)&
        print *, 'custom cooling'
        ntable = n_custom
        allocate(t_table(1:ntable))
        allocate(L_table(1:ntable))
        t_table(1:ntable) = t_custom(1:n_custom)
        L_table(1:ntable) = l_custom(1:n_custom)
  ...
  111       close(unitpar)
    ...
    ! w[iw]=w[iw]+qdt*S[wCT,x] where S is the source based on wCT within ixO
      ...
      case ('analytical')
        call cool_analytical(qdt, wCT, wCTprim, wnew)
      case default
...
! Only used for debugging.
    ...
    subroutine cool_analytical(qdt, wCT, wCTprim, wnew)
      !$acc routine seq
      use mod_global_parameters
      use mod_constants
      ...
      wnew(iw_e) = wnew(iw_e)-de
    end subroutine cool_analytical
